...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en" lang="en-us">
  ...
  <title>
      ...
      Configuration &middot; Ember CLI Mirage
...
<li>
  <a class="Desktop-nav__item active" href="/docs/v0.3.x/configuration/">
    Configuration
  </a>
</li>
  ...
  <div class="Docs-page">
    <div class='Docs-page__header'>
      <div class="Docs-page__version">
        <a class='Docs-page__improve' href="https://github.com/samselikoff/ember-cli-mirage/edit/gh-pages/docs/v0.3.x/configuration.md">
          <small>
      ...
      <h1 class='Docs-page__title'>Configuration</h1>
    </div>

    <div class='Docs-page__content'>
      <p>Besides defining your routes, there’s some config options for your server available in <code class="highlighter-rouge">mirage/config.js</code>. There’s also some environment options, which you define in <code class="highlighter-rouge">/config/environment.js</code>.</p>

<h2 id="server-config">Server config</h2>

<p>Note that <code class="highlighter-rouge">this</code> within your config function in <code class="highlighter-rouge">mirage/config.js</code> refers to the server instance, which is the same instance that <code class="highlighter-rouge">server</code> refers to in your tests.</p>
...
<p>If your Ember app is loaded from the filesystem vs. a server (e.g. via Cordova or Electron vs. <code class="highlighter-rouge">ember s</code> or <code class="highlighter-rouge">https://yourhost.com/</code>), you will need to explicitly define a namespace. Likely values are <code class="highlighter-rouge">/</code> (if requests are made with relative paths) or <code class="highlighter-rouge">https://yourhost.com/api/...</code> (if requests are made to a defined server).</p>

<p>For a sample implementation leveraging a configured API host &amp; namespace, check out <a href="https://github.com/samselikoff/ember-cli-mirage/issues/497#issuecomment-183458721">this issue comment</a></p>
...
<p>Note that this is a global parameter which affects all routes. You can override the timing for individual routes by using the <a href="../route-handlers#timing">route handler’s timing parameter</a>.</p>

<p><a name="logging" href="#logging">#</a> this.<b>logging</b></p>

<p>Set to <code class="highlighter-rouge">true</code> or <code class="highlighter-rouge">false</code> to explicitly specify logging behavior.</p>
...
<div class="language-js highlighter-rouge"><pre class="highlight"><code><span class="nx">test</span><span class="p">(</span><span class="s1">'I can view all users'</span><span class="p">,</span> <span class="kd">function</span><span class="p">()</span> <span class="p">{</span>
  <span class="nx">server</span><span class="p">.</span><span class="nx">logging</span> <span class="o">=</span> <span class="kc">true</span><span class="p">;</span>
  <span class="nx">server</span><span class="p">.</span><span class="nx">create</span><span class="p">(</span><span class="s1">'user'</span><span class="p">);</span>
...
<p>You can also write a custom log message, using the <a href="#pretender">Pretender server’s</a> <code class="highlighter-rouge">handledRequest</code> hook. See <a href="https://github.com/samselikoff/ember-cli-mirage/blob/2c31ad15a46e90b357a83b6896c6774fa42c6488/addon/server.js#L25">Mirage’s default implementation</a> for an example.</p>

<p>To override,</p>

<div class="language-js highlighter-rouge"><pre class="highlight"><code><span class="c1">// mirage/config.js</span>
<span class="kr">export</span> <span class="k">default</span> <span class="kd">function</span><span class="p">()</span> <span class="p">{</span>
  <span class="k">this</span><span class="p">.</span><span class="nx">pretender</span><span class="p">.</span><span class="nx">handledRequest</span> <span class="o">=</span> <span class="kd">function</span><span class="p">(</span><span class="nx">verb</span><span class="p">,</span> <span class="nx">path</span><span class="p">,</span> <span class="nx">request</span><span class="p">)</span> <span class="p">{</span>
    <span class="kd">let</span> <span class="p">{</span> <span class="nx">responseText</span> <span class="p">}</span> <span class="o">=</span> <span class="nx">request</span><span class="p">;</span>
    <span class="c1">// log request and response data</span>
  <span class="p">}</span>
<span class="p">}</span>
</code></pre>
</div>
...
<p>You can also pass a list of paths, or call <code class="highlighter-rouge">passthrough</code> multiple times:</p>
...
<p>In versions of Pretender prior to 0.12, <code class="highlighter-rouge">passthrough</code> only worked with jQuery &gt;= 2.x. As long as you’re on Pretender@0.12 or higher, you should be all set.</p>
...
<p>Set these options via the <code class="highlighter-rouge">ENV['ember-cli-mirage']</code> variable in your <code class="highlighter-rouge">config/environment.js</code> file:</p>
...
<p><a name="enabled" href="#enabled">#</a> <b>enabled</b></p>
...
<p><a name="excludeFilesFromBuild" href="#excludeFilesFromBuild">#</a> <b>excludeFilesFromBuild</b></p>

<p>Defaults to <code class="highlighter-rouge">false</code>.</p>

<p>By default, Mirage’s files are included in your Ember app’s build in non-production environments. This is in case you want to use Mirage via <code class="highlighter-rouge">ember serve</code> by visiting <code class="highlighter-rouge">/tests</code>, since that’s an app with a build-time environment of <code class="highlighter-rouge">development</code> but a run-time environment of <code class="highlighter-rouge">test</code>.</p>

<p>You can explicilty exclude Mirage’s files from your Ember app’s build by setting <code class="highlighter-rouge">excludeFilesFromBuild</code> to <code class="highlighter-rouge">true</code>.</p>

<p><a name="useDefaultPassthroughs" href="#useDefaultPassthroughs">#</a> <b>useDefaultPassthroughs</b></p>

<p>Defaults to <code class="highlighter-rouge">true</code>.</p>

<p>If true, Mirage will add some default passthrough routes to your server. Currently we add a single route</p>

<ul>
  <li>http://localhost:0/chromecheckurl</li>
</ul>

<p>which is used by iOS for URL verification.</p>

<p><a name="directory" href="#directory">#</a> <b>directory</b></p>

<p>Configure which directory contains your Mirage server definition. The default directory is <code class="highlighter-rouge">/mirage</code> (from the root of your project).</p>

<p>For example, to have your server definition under <code class="highlighter-rouge">/app/mirage</code>,</p>

<div class="language-js highlighter-rouge"><pre class="highlight"><code><span class="c1">// config/environment.js</span>
<span class="p">...</span>
<span class="nx">ENV</span><span class="p">[</span><span class="s1">'ember-cli-mirage'</span><span class="p">]</span> <span class="o">=</span> <span class="p">{</span>
  <span class="na">directory</span><span class="p">:</span> <span class="s1">'app/mirage'</span>
<span class="p">};</span>
</code></pre>
</div>
...
<div class='Docs-page-nav'>
         ...
         <a class="Docs-page-nav__link u-first" href="/docs/v0.3.x/acceptance-testing/">< Acceptance testing</a>
         ...
         <a class="Docs-page-nav__link u-last" href="/docs/v0.3.x/collections/">Collections ></a>
